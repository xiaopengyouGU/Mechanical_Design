...
protected:
	...
	double KHa = 1.2;
	double KHb = 1.2;			//接触疲劳对应的齿向载荷分配系数
	//齿轮的受力分析
	...
	virtual void Bend_Fatigue_Design();								//弯曲和接触疲劳强度设计
...
protected:
	...
	void setKT(double val) { KT = val; }							//设置试算系数
	...
	//setUserChoice函数的具体构成
	void setBasicParameter(std::ostream& os);						//基本参数，即不需查表的参数
	void setLifeParameter(std::ostream& os);						//寿命系数设定
	void setKHbParameter(std::ostream& os);							//齿向载荷分配系数,疲劳对应的可以直接查表
	//使用常量表达式和静态全局变量，优化内存
